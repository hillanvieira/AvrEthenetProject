...
#define ETH_CS_DDR       DDRB
#define ETH_CS           (1<<PORTB2)
#define ETH_CS_ENABLE()  (ETH_CS_PORT &= ~ETH_CS)
#define ETH_CS_DISABLE() (ETH_CS_PORT |=  ETH_CS)
...
#define TXBASEADDR  0x4000      // W5100 Send Buffer Base Address
...
//functions
extern void           SPI_Eth_Write(uint16_t addr,uint8_t data);
extern unsigned char  SPI_Eth_Read(uint16_t addr);
extern void           W5100_Init(uint8_t *ip_ptr, uint8_t *gtw_ptr, uint8_t *mask_ptr);
extern void           Eth_Close(uint8_t sock);
extern void           Eth_disconnect(uint8_t sock);
extern uint8_t        Eth_socket(uint8_t sock,uint8_t eth_protocol,uint16_t tcp_port);
extern uint8_t        Eth_Listen(uint8_t sock);
extern uint16_t       Eth_Send(uint8_t sock,const uint8_t *ethbuffer,uint16_t buffleng);
extern uint16_t       Eth_Recvr(uint8_t sock,uint8_t *ethbuffer,uint16_t buffleng);
extern uint16_t       Eth_Recvr_size(void);
extern int            Eth_cmp_Str_index(char *s,char *t);
